...
function yieldColor(yieldStr: string): string {
  if (yieldStr.includes("Ultra")) return "#10b981";
  if (yieldStr.includes("High")) return "#0F766E";
  if (yieldStr.includes("Medium")) return "#f59e0b";
  return "#dc2626";
}
...
export default function ProteinYieldChart({
  candidates,
}: {
  ...
  const maxYield = Math.max(10, ...candidates.map((c) => parseYield(c.predictedProteinYield)));
    ...
    <div className="w-full overflow-x-auto">
      ...
      >
        {[0, 2.5, 5, 7.5, 10].map((v) => {
          const x = PAD.left + (v / maxYield) * plotW;
          return (
            <g key={v}>
        ...
        <text x={PAD.left + plotW / 2} y={H - 5} textAnchor="middle" className="fill-black" fontSize={12} fontWeight={700}>
          Predicted Protein Yield (Fold Improvement)
        </text>

        {candidates.map((c, i) => {
          const val = parseYield(c.predictedProteinYield);
          const bw = (val / maxYield) * plotW;
          const y = rowY(i) - 10;
          ...
          return (
            <g key={c.constructId}>
              ...
              <rect x={PAD.left} y={y} width={Math.max(2, bw)} height={20} rx={4} fill={yieldColor(c.predictedProteinYield)} opacity={isHover ? 1 : 0.85} />
              ...
              {isHover && (
                <g>
                  ...
                  <text x={W - PAD.right - 232} y={PAD.top + i * rowH + 12} className="fill-slate-300" fontSize={12}>
                    Initiation {c.initiationEfficiency}% · CAI {c.cai.toFixed(2)} · MFE {c.mfe.toFixed(1)}
                  </text>
                </g>
              )}
            </g>
