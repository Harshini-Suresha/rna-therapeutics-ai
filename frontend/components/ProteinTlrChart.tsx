...
function riskColor(risk: string): string {
  if (risk.includes("Very Low")) return "#10b981";
  if (risk.includes("Low")) return "#0F766E";
  if (risk.includes("Moderate")) return "#f59e0b";
  if (risk.includes("High")) return "#ef4444";
  return "#64748b";
}

function tlrValue(risk: string): number {
  const m = risk.match(/(\d+(?:\.\d+)?)/);
  return m ? parseFloat(m[1]) : 10;
}

export default function ProteinTlrChart({
  candidates,
}: {
    ...
    <div className="w-full overflow-x-auto">
      ...
      >
        ...
        <text x={PAD.left + plotW / 2} y={H - 5} textAnchor="middle" className="fill-black" fontSize={12} fontWeight={700}>
          TLR Risk Score (combined TLR3 / 7 / 8)
        </text>

        {candidates.map((c, i) => {
          const val = tlrValue(c.tlrRisk);
          ...
          return (
            <g key={c.constructId}>
              ...
              <rect x={PAD.left} y={y} width={Math.max(2, bw)} height={20} rx={4} fill={riskColor(c.tlrRisk)} opacity={isHover ? 1 : 0.85} />
              ...
              {isHover && (
                <g>
                  ...
                  <text x={W - PAD.right - 232} y={PAD.top + i * rowH + 12} className="fill-slate-300" fontSize={12}>
                    TLR3 {c.diagnostics.tlr3Score} · TLR7 {c.diagnostics.tlr7Score} · TLR8 {c.diagnostics.tlr8Score}
                  </text>
                </g>
              )}
            </g>
