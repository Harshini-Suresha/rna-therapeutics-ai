...
export default function ProteinReplacementDeepDiveCard({
  candidate,
  rank,
}: {
    ...
    <Card className="p-5">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-3">
          ...
          <p
            className={`text-[14px] font-bold ${
              candidate.initiationEfficiency >= 90
                ? "text-emerald-600"
                : candidate.initiationEfficiency >= 80
                  ? "text-amber-600"
                  : "text-red-600"
            }`}
          >
            {candidate.initiationEfficiency}%
          </p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-3">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 mb-1">
          ...
          <p
            className={`text-[14px] font-bold ${
              candidate.predictedProteinYield.includes("Ultra")
                ? "text-emerald-600"
                : candidate.predictedProteinYield.includes("High")
                  ? "text-blue-600"
                  : "text-amber-600"
            }`}
          >
            {candidate.predictedProteinYield}
          </p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-3">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 mb-1">
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-center">
          ...
          <p className="text-[12px] font-bold text-slate-700">{candidate.diagnostics.tlr3Score}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-center">
          <p className="text-[10px] text-slate-400 mb-0.5">TLR7</p>
          <p className="text-[12px] font-bold text-slate-700">{candidate.diagnostics.tlr7Score}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-center">
          <p className="text-[10px] text-slate-400 mb-0.5">TLR8</p>
          <p className="text-[12px] font-bold text-slate-700">{candidate.diagnostics.tlr8Score}</p>
        </div>
      </div>
