...
export default function ProteinInitiationChart({
  candidates,
}: {
  ...
  return (
    <div className="w-full overflow-x-auto">
      ...
      >
        ...
        {candidates.map((c, i) => {
          const bw = (c.initiationEfficiency / maxScore) * plotW;
          const y = rowY(i) - 10;
          ...
          return (
            <g key={c.constructId}>
              ...
              <rect x={PAD.left} y={y} width={Math.max(2, bw)} height={20} rx={4} fill={scoreColor(c.initiationEfficiency)} opacity={isHover ? 1 : 0.85} />
              <text x={Math.max(bw + 8, 40)} y={rowY(i) + 4.5} className="fill-slate-700" fontSize={13} fontWeight={600}>
                {c.initiationEfficiency}%
              </text>
              {isHover && (
                <g>
                  ...
                  <text x={W - PAD.right - 232} y={PAD.top + i * rowH + 12} className="fill-slate-300" fontSize={12}>
                    CAI {c.cai.toFixed(2)} · U% {c.uContent.toFixed(1)} · MFE {c.mfe.toFixed(1)}
                  </text>
                </g>
              )}
            </g>
