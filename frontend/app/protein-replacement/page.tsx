...
import {
  fetchDesignOptions,
  generateConstructs,
} from "@/lib/proteinReplacementApi";

const CONFIRMED_TARGET_KEY = "aso:confirmedTarget";

export default function ProteinReplacementPage() {
  ...
  const [results, setResults] = useState<ProteinReplacementResponse | null>(null);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setResults(res);
      saveReport({
        step: "protein_replacement",
        title: `Protein Replacement: ${targetSymbol.toUpperCase()}`,
        geneSymbol: targetSymbol.toUpperCase(),
        disease: gene?.disease || "",
        summary: `Generated ${res.candidates.length} RNA constructs for ${targetSymbol.toUpperCase()}. Top: ${res.candidates[0]?.constructId || "N/A"}.`,
        data: {
          modality: rnaModality,
          topCandidates: res.candidates.slice(0, 3).map((c) => ({
            constructId: c.constructId,
            modality: c.modality,
            cai: c.cai,
            yield: c.predictedProteinYield,
          })),
        },
      });
    } catch (err) {
  ...
  function handleSelectCandidate(candidate: RnaCandidate) {
  ...
  function isFormValid(): boolean {
    return (
      ...
      !!utrPair &&
      !!nucleotideModification &&
      (rnaModality !== "circrna" || !!iresSelection)
    );
  }
    ...
    <div className="flex min-h-screen bg-[#F5F6FA]">
      ...
      <div className="flex min-h-screen flex-1 flex-col">
        ...
        <main className="flex-1 space-y-5 px-6 py-6">
          ...
          {/* Step 1: Pre-Start Design Form */}
          {!results && (
            <Card>
              <SectionHeader step="1" title="Pre-Start Design Options" />
              <p className="px-6 pb-3 text-[12.5px] text-slate-500">
                Configure the RNA payload parameters for protein replacement. All fields are required unless noted otherwise.
              </p>
              {optionsError && (
              ...
              <div className="grid grid-cols-1 gap-4 px-6 pb-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                <div>
                  ...
                  {rnaModality && (
                    <p className="mt-1 text-[10.5px] text-slate-400">
                      {options?.rnaModalities.find((o) => o.id === rnaModality)?.description}
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {options?.nucleotideModifications.map((o) => (
                      <option key={o.id} value={o.id}>
                        {o.label}
                      </option>
                    ))}
          ...
          {results && (
            <>
              {/* Step 2: Header & Construct Overview */}
              <Card>
                <SectionHeader
                  step="2"
                ...
                <div className="px-6 pb-5">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div className="space-y-3">
                      ...
                      <InfoField label="Selected RNA Modality" value={results.overview.vectorTopology} />
                      <InfoField label="Native Protein Length" value={results.overview.nativeLength} />
                    </div>
                    <div className="space-y-3">
                      <InfoField
                        label="Codon Adaptation Index (CAI)"
                        value={results.overview.cai.toFixed(2)}
                        valueClassName={results.overview.cai >= 0.92 ? "text-emerald-600" : "text-amber-600"}
                      />
                      <InfoField
                        label="Uridine Percentage (U%)"
                        value={`${results.overview.uContent.toFixed(1)}%`}
                        valueClassName={results.overview.uContent < 20 ? "text-emerald-600" : "text-amber-600"}
                      />
                    ...
                    <div className="space-y-3">
                      ...
                      <InfoField
                        label="Expression Feasibility Score"
                        value={`${results.overview.feasibilityScore}/100`}
                        valueClassName={results.overview.feasibilityScore >= 80 ? "text-emerald-600" : "text-amber-600"}
                      />
                    </div>
                  </div>
                </div>
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="border-b-2 border-slate-200">
                        ...
                        <th className="pb-2 pr-3 text-[10px] font-bold uppercase tracking-wider text-slate-400 text-right">MFE (kcal/mol)</th>
                      ...
                      {results.candidates.map((c) => (
                        <tr
                          ...
                          className={`border-b border-slate-100 last:border-0 hover:bg-slate-50/50 transition-colors cursor-pointer ${
                            selectedCandidate?.constructId === c.constructId ? "bg-brand/5" : ""
                          }`}
                        ...
                        >
                          ...
                          <td className="py-3 pr-3">
                            <Pill tone={c.predictedProteinYield.includes("Ultra") ? "green" : c.predictedProteinYield.includes("High") ? "blue" : "amber"}>
                          ...
                          <td className="py-3 pr-3">
                            <span
                              className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-semibold ${
                                c.tlrRisk.includes("Very Low")
                                  ? "border-emerald-200 bg-emerald-50 text-emerald-700"
                                  : c.tlrRisk.includes("Low")
                                    ? "border-blue-200 bg-blue-50 text-blue-700"
                                    : c.tlrRisk.includes("Moderate")
                          ...
                          <td className="py-3 pr-3">
                            <span
                              className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-semibold ${
                                c.secondaryStructureFlag === "PASSED"
                                  ? "border-emerald-200 bg-emerald-50 text-emerald-700"
                                  : "border-red-200 bg-red-50 text-red-700"
                              }`}
                            >
                ...
                <Card className="overflow-hidden">
                  <div className="flex items-center justify-between px-6 pt-4 pb-3 border-b border-slate-100">
                    ...
                    <button
                      ...
                      className="flex h-6 w-6 items-center justify-center rounded-full border border-slate-200 text-slate-400 hover:text-slate-600"
                    >
                      <X className="h-3.5 w-3.5" />
                    </button>
                  </div>
                  <div className="px-6 pb-5 space-y-5">
                    {/* Interactive Full Construct Schematic */}
                    <div>
                      <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 mb-2">
                        Full Transcript Feature Map
                      </p>
                        ...
                        <div className="flex items-center gap-1 min-w-[600px]">
                          {selectedCandidate.features.map((f, i) => (
                            <div key={i} className="flex items-center gap-1">
                              <div
                                className={`flex flex-col items-center rounded-md px-2 py-1.5 text-[10px] font-medium text-center min-w-[70px] ${
                                  f.type === "cap"
                              ...
                              {i < selectedCandidate.features.length - 1 && (
                    ...
                    <div>
                      <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 mb-2">
                        Construct Transcript Sequence (5&apos; → 3&apos;)
                      </p>
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => copySequence(selectedCandidate.sequence)}
                          className="flex shrink-0 items-center gap-1 rounded-md border border-slate-200 px-2.5 py-1.5 text-[11px] text-slate-500 hover:bg-white transition-colors"
                        >
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                      <div className="rounded-lg border border-slate-200 bg-white p-3">
                        ...
                        <p className="text-[14px] font-bold text-emerald-600">
                          {selectedCandidate.diagnostics.aminoAcidIdentity}%
                        </p>
                      ...
                      <div className="rounded-lg border border-slate-200 bg-white p-3">
                        ...
                        <p className="text-[14px] font-bold text-slate-700">{selectedCandidate.diagnostics.tlr3Score}</p>
                      ...
                      <div className="rounded-lg border border-slate-200 bg-white p-3">
                        ...
                        <p className="text-[14px] font-bold text-slate-700">
                          {selectedCandidate.diagnostics.tlr7Score} / {selectedCandidate.diagnostics.tlr8Score}
                        </p>
                    ...
                    <div>
                      <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 mb-2">
                        5&apos; UTR / Ribosome Entry Secondary Structure (ViennaRNA MFE)
                      </p>
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        <span
                          className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-semibold ${
                            selectedCandidate.diagnostics.fiveUtrHairpin
                              ? "border-red-200 bg-red-50 text-red-700"
                              : "border-emerald-200 bg-emerald-50 text-emerald-700"
                          }`}
                        >
                          {selectedCandidate.diagnostics.fiveUtrHairpin
                            ? "FAILED — 5' UTR Hairpin Obstacle Detected"
                ...
                <Card>
                  ...
                  <div className="px-6 pb-5 flex flex-wrap items-center gap-3">
                    <button
                      onClick={() => {
                        const blob = new Blob(
                          [results.candidates.map((c) => `>${c.constructId}\n${c.sequence}`).join("\n\n")],
                          { type: "text/plain" }
                        );
                        const url = URL.createObjectURL(blob);
                        const a = document.createElement("a");
                        a.href = url;
                        a.download = `${targetSymbol}_constructs.fasta`;
                        a.click();
                    ...
                    >
                      ...
                      Export Candidate Sequences to FASTA
                    </button>
                    <button
                      onClick={() => alert("LNP Formulation & Payload Optimization module is under development.")}
                      className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-[13px] font-medium text-slate-700 hover:bg-slate-50 transition-colors"
                    >
                      <FlaskConical className="h-4 w-4" />
                      Proceed to LNP Formulation &amp; Payload Optimization
                    </button>
                    <button
                      onClick={() => alert("IVT Plasmid Template & Synthesis Protocol generation is under development.")}
                      className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-[13px] font-medium text-slate-700 hover:bg-slate-50 transition-colors"
                    >
                      <FileText className="h-4 w-4" />
                      Generate IVT Plasmid Template &amp; Synthesis Protocol
                    </button>
                  </div>
                </Card>
              )}
